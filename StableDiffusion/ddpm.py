...
class DDPMSampler:
    ...
    def set_inference_steps(self, num_inference_steps=50):
        self.num_interfence_steps = num_inference_steps
        step_ratio = self.num_training_steps // self.num_interfence_steps
        timesteps = (np.arange(0, num_inference_steps) * step_ratio).round()[::-1].copy().astype(np.int64)
        self.timesteps = torch.from_numpy(timesteps)

    def _get_previous_timestep(self, timestep: int) -> int:
        prev_t = timestep - (self.num_training_steps // self.num_interfence_steps)
        return prev_t
    ...
    def set_strength(self, strength=1):
        start_step = self.num_interfence_steps - int(self.num_training_steps * strength)
